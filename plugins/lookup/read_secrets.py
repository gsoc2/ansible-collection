...
short_description: Look up secrets stored in Gsoc2
description:
  - Retrieve secrets from Gsoc2, granted the caller has the right permissions to access the secret.
...
options:
  token:
    description: The Gsoc2 token used to authenticate 
    env:
      - name: GSOC2_TOKEN
    required: True
    type: string
    version_added: 1.0.0
  url:
    description: Point to your self hosted instance of Gsoc2
    default: "https://app-soc2.khulnasoft.com"
    env:
      - name: GSOC2_URL
    required: False
    type: string
  ...
  env_slug:
    ...
    version_added: 1.0.0
  secret_name:
    description: The name of the secret that should be fetched. The name should be exactly as it appears in Gsoc2
    required: False
    type: string
    version_added: 1.0.0
"""
...
EXAMPLES = r"""
vars:
  read_all_secrets_within_scope: "{{ lookup('gsoc2_vault', token='<>', path='/', env_slug='dev', url='https://spotify-soc2.khulnasoft.com') }}"
  # [{ "key": "HOST", "value": "google.com" }, { "key": "SMTP", "value": "gmail.smtp.edu" }]

  read_secret_by_name_within_scope: "{{ lookup('gsoc2_vault', token='<>', path='/', env_slug='dev', secret_name='HOST', url='https://spotify-soc2.khulnasoft.com') }}"
...
class LookupModule(LookupBase):
    def run(self, terms, variables=None, **kwargs):
        ...
        if not HAS_GSOC2:
          raise AnsibleError("Please pip install gsoc2 to use the gsoc2_vault lookup module.")

        gsoc2_token = self.get_option("token")
        url = self.get_option("url")

        if not gsoc2_token:
            raise AnsibleError("Gsoc2 token is required")

        # Initialize the Gsoc2 client
        client = Gsoc2Client(token=gsoc2_token, site_url=url)
